            ...
            tx4.executeSql("SELECT r.status, r.idUser, r.idCuestionario AS cuestionarioId, au.user_id, au.idCuestionario FROM register as r LEFT OUTER JOIN answer_user as au on cuestionarioId = au.idCuestionario WHERE user_id = ? AND r.status = 1 GROUP BY cuestionarioId",
                ...
                function (tx4, results) {
                    ...
                    for (var i = 0; i < length; i++) {
                        ...
                        //     }
                        // );
                        storeQuestions(item2.cuestionarioId); //Ahora si hay cedulas en estatus 1 o 2 va a llamar al metodo de enviar ahora si  donde le pasaremos el ID que identifica a todo
                        //Y LISTO ;)  oooh va va simon we ;)
                        //y dice la jefita que si le puede envia runa foto de los puntos que ayer anotaron plis
                        //va we eso es todo me salgo  jajaja va
                        //XD
                    }
                },
                function (tx4, error) {
